//
//  ContenidoParticipacionCiudadana.swift
//  appCpccs
//
...
class ContenidoParticipacionCiudadana{
    var id: Int!
    var titulo: String!
    ...
    class func getContenido(data:NSData)-> Array<ContenidoParticipacionCiudadana>{
        var contenidos: [ContenidoParticipacionCiudadana] = []
        ...
        if let contenidosArray = itemsContenidos as? NSArray{
            contenidosArray.enumerateObjectsUsingBlock({model, index, stop in
                ...
                var contenidoPC: ContenidoParticipacionCiudadana = ContenidoParticipacionCiudadana(id: id, titulo: titulo1,descripcion:descripcion,link:link)
                if(seccion=="PC"){
